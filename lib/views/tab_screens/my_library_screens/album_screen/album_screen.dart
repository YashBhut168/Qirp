...
class _AlbumScreenState extends State<AlbumScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Obx(() => albumScreenController.isLoading.value == true
          ? Center(
              child: CircularProgressIndicator(
                color: AppColors.white,
              ),
            )
          : albumScreenController.albumData.isEmpty
              ? Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      lable(
                          text:
                              "Sorry, the list of album is currently unavailable."),
                    ],
                  ),
                )
                    ...
                    child: Column(
                      children: [
                        ...
                        GridView.builder(
                          ...
                          itemCount: albumScreenController
                              .albumData.length, // total number of items
                          itemBuilder: (context, index) {
                            var albumListData =
                                albumScreenController.albumData[index];
                            return GestureDetector(
                              onTap: () {
                                setState(() {
                                  GlobVar.albumId = albumListData.id;
                                  if (kDebugMode) {
                                    print(
                                        'GlobVar.albumId----> ${GlobVar.albumId}');
                                  }
                                });
                                albumScreenController.albumsSongsList(
                                    albumId: albumListData.id);
                                Get.to(const AlbumSongsScreen());
                              },
                              child: Column(
                                children: [
                                  Stack(
                                    children: [
                                      CachedNetworkImage(
                                        imageUrl: (albumListData.picture),
                                      ...
                                      Positioned.fill(
                                        ...
                                        child: Container(
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              lable(
                                                text: (albumListData.name)!,
                                                  fontWeight: FontWeight.w600,
                                                  fontSize: 12,
                                                ),
                                              lable(
                                                text: "${(albumListData.songCount!)} songs",
                                                  fontWeight: FontWeight.w600,
                                                  fontSize: 11,
                                                  color: Colors.grey,
                                                ),
